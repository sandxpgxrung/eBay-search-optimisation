...
def filter():
    ...
    item_dictionary = search_items(request.args.get('search' or 'piano'))
    input_limit = request.args.get('setPrice') or 900
    ...
    def get_fitness(genome: Genome, items: [item_structure], price_limit: int) -> int:
    ...
    def run_evolution(populate_func: PopulateFunc, fitness_func: FitnessFunc, fitness_limit: int,
                      ...
                      mutation_func: MutationFunc = get_mutation, generation_limit: int = 100) -> Tuple[
        Population, int]:
    ...
    solutions = genome_to_items(population[1], items)
    return render_template("filter.html", solutions=solutions)
